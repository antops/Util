...
bool parseJsonToGraphTrans(const Json::Value & js, GraphTrans & graphTrans, double scale)
{
	...
	graphTrans.setGraphTransPar(
		js["trans_x"].asDouble() * scale,
		js["trans_y"].asDouble() * scale,
		js["trans_z"].asDouble() * scale,
