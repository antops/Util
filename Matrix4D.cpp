...
Matrix4D Matrix4D::getRotateMatrix(double angle, double x, double y, double z)
{
	angle = angle / 2 * M_PI / 180;
	Vector3 vec(x, y, z);
	vec.Normalization();
...
Matrix4D Matrix4D::getRotateMatrixRad(double theta, double phi)
{
	...
	Matrix4D matrix;
	matrix.m[0][0] = cost*cosp;
	matrix.m[0][1] = cost*sinp;
	matrix.m[0][2] = -sint;
	...
	matrix.m[2][0] = sint*cosp;
	matrix.m[2][1] = sint*sinp;
	matrix.m[2][2] = cost;
...
Matrix4D Matrix4D::getInvRotateMatrixRad(double theta, double phi)
{
	double cost = cos(theta);
	double sint = sin(theta);
	double cosp = cos(phi);
	...
	Matrix4D matrix;
	matrix.m[0][0] = cosp*cost;
	matrix.m[0][1] = -sinp;
	matrix.m[0][2] = cosp*sint;

	matrix.m[1][0] = sinp*cost;
	matrix.m[1][1] = cosp;
	matrix.m[1][2] = sinp*sint;
